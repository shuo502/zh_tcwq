...
$info=pdo_get('zhtc_ad',array('id'=>$_GPC['id']));
$city=pdo_getall('zhtc_hotcity',array('uniacid'=>$_W['uniacid']),array(),'','time ASC');
if(checksubmit('submit')){
     ...
     if($_GPC['id']==''){  
        $res=pdo_insert('zhtc_ad',$data);
        if($res){
             message('添加成功！', $this->createWebUrl('inad'), 'success');
        }else{
             message('添加失败！','','error');
        }
    }else{
...
include $this->template('web/inaddad');