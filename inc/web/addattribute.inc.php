...
$GLOBALS['frames'] = $this->getMainMenu();
	$info = pdo_get('zhtc_goods_spec',array('uniacid' => $_W['uniacid'],'id'=>$_GPC['id']));
		if(checksubmit('submit')){
			...
			if($_GPC['id']==''){				
				$res=pdo_insert('zhtc_goods_spec',$data);
				if($res){
				...
				$res = pdo_update('zhtc_goods_spec', $data, array('id' => $_GPC['id']));
				if($res){
