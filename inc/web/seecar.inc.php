...
$info=pdo_get('zhtc_car',array('id'=>$_GPC['id']));
if(checksubmit('submit')){
			...
			$data['other']=$_GPC['other'];
				...
				$res = pdo_update('zhtc_car', $data, array('id' => $_GPC['id']));
				if($res){
					message('编辑成功',$this->createWebUrl('carinfo',array()),'success');
				}else{
					message('编辑失败','','error');
				}
		}
include $this->template('web/seecar');