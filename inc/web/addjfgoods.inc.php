...
$GLOBALS['frames'] = $this->getMainMenu();
	$info = pdo_get('zhtc_jfgoods',array('uniacid' => $_W['uniacid'],'id'=>$_GPC['id']));
	$type = pdo_getall('zhtc_jftype',array('uniacid' => $_W['uniacid']));
		if(checksubmit('submit')){
			...
			if($_GPC['id']==''){				
				$res=pdo_insert('zhtc_jfgoods',$data);
				if($res){
				...
				$res = pdo_update('zhtc_jfgoods', $data, array('id' => $_GPC['id']));
				if($res){
