...
if($_GPC['op']=='delete'){
    ...
    if($res){
         message('删除成功！', $this->createWebUrl2('dlinstore'), 'success');
        }else{
              message('删除失败！','','error');
        }
}
if($_GPC['op']=='tg'){
  ...
  $time2=time();
   $res=pdo_update('zhtc_store',array('state'=>2,'sh_time'=>$time2,'dq_time'=>time()+$time),array('id'=>$_GPC['id']));
    ...
    if($res){
        message('通过成功！', $this->createWebUrl2('dlinstore'), 'success');
    }else{
        message('通过失败！','','error');
    }
}
if($_GPC['op']=='jj'){
    ...
    if($res){
     message('拒绝成功！', $this->createWebUrl2('dlinstore'), 'success');
    }else{
          message('拒绝失败！','','error');
    }
}
include $this->template('web/dlinstore');